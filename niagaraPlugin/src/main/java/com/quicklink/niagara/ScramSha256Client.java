...
/**
 * com.github.unldenis.test.ScramSha256Client is a utility class for managing the creation and
 * processing of messages used during a ScramSha256 handshake. This version of the class has been
 * simplified. Things that can be improved:
 * <p>
 * - Switch from using a String for password to a byte[] or char[] since they can be cleared prior
 * to garbage collection
 * <p>
 * - For the string and byte[] comparisons, use a constant time comparison algorythm instead of the
 * standard ones in java
 * <p>
 * - Creating a cache for the salted password so that repeated recalculations aren't necessary. This
 * needs to be balanced with storing sensitive data in memory.
 * <p>
 * NOTES:
 * <p>
 * When a problem with the data is detected (generally due to invalid credentials, etc.), a
 * SecurityException is thrown. The reason no messages are attached to the security exception is
 * that the client should not be told why it failed since that might provide hints to an attacker.
 * <p>
 * Refer to RFC5802 (https://tools.ietf.org/html/rfc5802) for details regarding the ScramSha
 * handshake mechanism.
 */
public final class ScramSha256Client {
  ...
  public String createClientFirstMessage() {
    try {
      ...
      StringBuffer buf = new StringBuffer("n,,").append(clientFirstMessageBare);
  ...
  /**
   * based on the contents of the serverFirstMessage, creates the client-final-message that can be
   * send back to the server, or throws an exception if things aren't proceeding correctly
   *
  ...
  public String createClientFinalMessage(String serverFirstMessage) {
    try {
      ...
      clientFinalMessageWithoutProof = _createClientFinalMessageWithoutProof(this.clientNonce,
          serverNonce);
      authMessage = _createAuthMessage(clientFirstMessageBare, serverFirstMessage,
  ...
  /**
   * parses the final message from the server and performs the last validation as defined in
   * RFC5802
   *
  ...
  public void processServerFinalMessage(String serverFinalMessage) {
    try {
      ...
      //#ifdef DEBUG
      debug("comparing server signature " + bytesToHex(serverSignature) + " <=> " + bytesToHex(
          remoteServerSignature));
      //#endif
  ...
  /**
   * standardizes the username string for languages that have multiple definitions for the same
   * character sets
   *
   * @param userName
  ...
  /**
   * standardizes the password string for languages that have multiple definitions for the same
   * character sets
   *
   * @param password
   * @return normalized username
   ...
   * @throws NoSuchAlgorithmException
   */
  private final byte[] xor(byte[] left, byte[] right)
   ...
   * @return
   */
  private final String _createClientFinalMessageWithoutProof(String clientNonce,
      String serverNonce) {
    StringBuffer buf = new StringBuffer("c=biws,r=");
   ...
   * @return
   */
  private final String _createAuthMessage(String clientFirstMessageBare, String serverFirstMessage,
      String clientFinalMessageWithoutProof) {
    StringBuffer buf = new StringBuffer(clientFirstMessageBare);
    buf.append(",").append(serverFirstMessage).append(",").append(clientFinalMessageWithoutProof);
    return buf.toString();
  }

  /**
   * calculate the server signature derived from the auth message and salted password per Rfc5802
   *
   * @param saltedPassword
   * @param authMessage
   * @return
  ...
  private static void debug(String msg) {
    if (debugFlag) {
      System.err.println("[com.github.unldenis.test.ScramSha256Client] " + msg);
    }
  }
  //#endif
